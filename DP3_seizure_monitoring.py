    ...
    def run(self):
        try:
            ...
            ************************************************************
            ...
            ## Please read the READ ME FIRST.txt file before proceeding.
            
            '''
            This Program is designed to detect seizures, identify types of seizure, measure length of the seizures and the time of day in infants.
            The program will write to two different files throughout its implementation: 
		...
		2- seizure details: inlcudes the list of rolling averages of the past 30 seconds, Seizure status (Type of seizure), time of day & length of seizure.
            It will also write to the Shell the raw data, Rolling average and the status of the output devices.
            '''

            
            #Prints headings to shell and filename_1 for status of output devices.
            print("Voltage (raw) \t Voltage (avg) \t Buzzer \t LED \n") 
            write_to_file("Voltage (raw) \t Voltage (avg) \t Status \n", filename_1)

            while True:
                #Ensure all outputs are off for each cycle
                green_led.off()
                red_led.off()
                blue_led.off()
                yellow_led.off()
                ...
                global raw_point
                raw_point = sensor.muscle_raw()
                time.sleep(data_time)

                #Calls the rolling_average function
                global average
                average = rolling_average(n)
    
                #If the Rolling average function returns no value, the parents are alerted that the device is disconnected (no readings).
                
                if average == None:
                    yellow_led.on()
                    led_status = "Yellow"
                    status = "Device not connected"
                    line = str(raw_point) + "\t" + str(average) + "\t" + str(status) + "\n"
                    write_to_file(line, filename_1)
                    print_shell(raw_point, average, buzzer_status, led_status)

                else:
                    seizure_time = datetime.datetime.now()
                    seizure = process_1(average)

                    #The rolling average, time of day are appened to a file for potential analysis. If a seizure is detected, "Threshold 1 Passed: Possible Seizure" is appended.
                    # Output devices buzzer, yellow and green LED lights are turned on.
                    if seizure == True:
                        buzzer.on()
                        yellow_led.on()
                        green_led.on()
                        buzzer_status = "On"
                        led_status = "Yellow + Green"
                        status = "Threshold 1 passed: Possible Seizure"
...
filename_1 = "raw_data"
filename_2 = "seizure_details"

threshold_1 = 150 #Initial/tonic threshold
threshold_2 = 100 #False Alarm Threshold
threshold_3 = 75 #ROC threshold

n = 3 # No. of raw data points that will be taken the average of for the rolling average
data_time = 0.2 #Time in between each reading of raw data.
time_check = 30 #The lower bound of length of seizures.
...
#The Rolling Average Function appends the last n raw data readings from the EMG sensor to the Value list with data_time time intervals.
def rolling_average(n):
    values = []
    for i in range(n): # This function take n*data_time seconds for each average
        data_point = sensor.muscle_raw()
        values.append(data_point)
        time.sleep(data_time)
    #The average of the iterables in the list is calculated and returned.  
...
#The blinking_lights function takes the number of cycles as input. This function is used to alert the parents if their child had a seizure for longer than 5 minutes (which is a medical emergency).
def blinking_lights(cycles):
    if cycles >= 10:   #If the number of cycle reaches/exceeds 10 cycles, a buzzer is turned on and blinking red & blue lights are turned on to alert the parent of an emergency. 
        global led_status
        global buzzer_status
        global status
        buzzer.on()
        buzzer_status = "On"
        led_status = "Red + Blue"
        status = "Medical Emergency: seizure time > 5 min"   #Writes to Shell & filename_1
        print_shell(raw_point, average, buzzer_status, led_status)
        line = str(raw_point) + "\t" + str(average) + "\t" + str(status) + "\n"
        write_to_file(line, filename_1)
        for i in range (20):
...
#The alert_values function creats a list (alert_list) of the last "time_check/(n*data_time)" rolling averages which will be used furthur analysis.
        #Each average takes n*data_time seconds, to check how many averages were returned in time_check seconds, we have to divide time_check/(n*data_time).
def alert_values():
    global average
    global raw_point
    alert_list = []
    for i in range(int(time_check*(1/(n*data_time)))): #This for loop iterates 50 times.
        seizure_time = datetime.datetime.now()
...
#This funcation appends "value" to  file "filename"
def write_to_file(value, filename):
    with open(filename, 'a') as file:          
        file.write(value)

#list_average calculates the average of any input list. It also excepts TypeErros in the case where "None is a value in the list, and removes the "None" value to reduce error in average calculated.
def list_average(input_list):
    try:
        avg_list = sum(input_list)/len(input_list)
        return avg_list
    except TypeError:
        while None in input_list: input_list.remove(None)
        print("Device has been temporarily disconnected")
        avg_list = sum(input_list)/len(input_list)
        return avg_list
    except ZeroDivisionError: #In case of all the values = none, thus leaving an empty list to be divided by 0.
        print("Warning: Device disconnected")



 #The Rate of change between neigbouring values in the input list is calculated. 
def comp_roc(input_list):
    roc_list = []
    for i in range(1,len(input_list)):
        roc = abs((input_list[i]-input_list[i-1])/(n*data_time)) # The Absolute value of rate of change is appended to roc_list.
        roc_list.append(roc)
    return roc_list #roc_list is returned



###The functions below (false_alarm(), tonic() ,clonic(), unidentified()) return the status of the LED lights. At the begining of each function all the leds that are not in use are turned off



#If the average of alert values is less than Threshold_2, then it was a false alarm.
def false_alarm(alert_average):
    if alert_average < threshold_2:
        blue_led.off()
        red_led.off()
        print("False alarm")
        green_led.on()#Turns on the Green LED lights
        return True
    return False

#If the lowest value (index[0]) in the sorted list is greater than threshold_1, then a Tonic Seizure has been detected.
def tonic(sorted_list):
    if sorted_list[0] > threshold_1:
        red_led.off() 
        green_led.off()
        yellow_led.off()
        #Tonic Seizure
        print("Tonic seizure")
        blue_led.on() #Turns on the Blue LED lights
        return True
    return False

#If the average rate of change of rolling averages is greater than threshold_3, then a Clonic Seizure has been detected.
def clonic(roc_average):
    if roc_average > threshold_3:
        blue_led.off()
        green_led.off()
        yellow_led.off()
        print("Clonic seizure")
        red_led.on()#Turns on the red LED lights
        return True
    return False

#If none of the above functions were able to identify the seizure and the average of the alert values is above threshold_2, then the program is unable to identify the seizure.
def unidentified(alert_average):
    if alert_average > threshold_2:
        red_led.off()
        blue_led.off()
        yellow_led.on()#Turns on the Yellow and Green LED lights
        green_led.on()
        print("Unable to identify seizure")
        return True
    return False
...
#This function is used to simplify the code involving outputting to the shell
def print_shell(raw_point, average, buzzer_status, led_status):
    shell_line = str(raw_point) + "\t" + str(average) + "\t" + buzzer_status + "\t" + led_status + "\n" 
    print(shell_line)



#Process one evaluates whether the average from the rolling average function is within normal range of average by comparing it to the abnormal threshold.
def process_1(average):
    if average < threshold_1:#If value below threshold_1, then the function returns False
        return False
    else: #Otherwise, returns True.
        return True
    

#Process two analyzes the overall pattern of the the rolling averages to determine what type of seizure, Length of seizure, and whether is was a false alarm.
def process_2():
    global led_status
    global status
    
    #Initializes an empty list and then assigns it to the function alert_value which returns a list of the rolling averages.
    alert_list = []
    alert_list = alert_values()

    #Assigns and defines the the variable alert_average to the function list_average, which returns the average of the iterables of a list.
    #This gives us the average of the rolling averages in alert_list
    alert_average = list_average(alert_list)

    #We first assess whether this is a an actual seizure and not a false alarm by comparing the returned statement from false_alarm().

    #If false_alarm() returns "Green LED is on", a false alarm has been detected.
    if false_alarm(alert_average) == True:
        led_status = "Green"
        status = "False Alarm"
        print_shell(raw_point, average, buzzer_status, led_status)
        line = str(raw_point) + "\t" + str(average) + "\t" + str(status) + "\n"
    ...
    else: #Otherwise there is a seizure,we will assess what type of seizure is occuring.
        while True:
            seizure_time = datetime.datetime.now()
    
            #Each time the program goes through the while True loop, one is added to the number of cycles which will represent the number of loops through the While True loop until it breaks out.
            global cycles
            cycles += 1

            #The alert_list is sorted from low to high.
            sorted_list = sorted(alert_list)

            #We make a list of the rate of change between of the values in alert_list
            roc_list = comp_roc(alert_list)

            #Calculates the average of the values in roc_list using the list_average()function.
            roc_average = list_average(roc_list) 

            #The returned statuses of the LEDS of the functions tonic(), clonic() and undentified()are compared to identify the type of seizure.
            #In each case, the information is written to two files, and the python shell.

            if tonic(sorted_list) == True:
                led_status = "Blue"
                status = "Tonic Seizure"
                line = "Values: " + str(alert_list) + '\n' + str(seizure_time.strftime("%X \t %x")) + '\t' + 'Tonic Seizure' + '\n'
                write_to_file(line, filename_2)
                print_shell(raw_point, average, buzzer_status, led_status)
                line = str(raw_point) + "\t" + str(average) + "\t" + str(status) + "\n"
                write_to_file(line, filename_1)

            elif clonic(roc_average) == True:
                led_status = "Red"
                status = "Clonic Seizure"
                line = "Values: " + str(alert_list) + '\n' + str(seizure_time.strftime("%X \t %x")) + '\t' + 'Clonic Seizure' + '\n'
                write_to_file(line, filename_2)
                print_shell(raw_point, average, buzzer_status, led_status)
                line = str(raw_point) + "\t" + str(average) + "\t" + str(status) + "\n"
                write_to_file(line, filename_1)

            elif unidentified(alert_average) == True:
                led_status = "Yellow + Green"
                status = "Unidentified Seizure"
                line = "Values: " + str(alert_list) + '\n' + str(seizure_time.strftime("%X \t %x")) + '\t' + 'Type of Seizure is undentified' + '\n'
                write_to_file(line, filename_2)
                print_shell(raw_point, average, buzzer_status, led_status) 
                line = str(raw_point) + "\t" + str(average) + "\t" + str(status) + "\n"
                write_to_file(line, filename_1)               

            else: #If none of the conditions are met above, then the seizure has come to an end.
                led_status = "Off"
                seizure_length = cycles * time_check
                status = "Seizure has stopped: Length = " + str(seizure_length)
                print_shell(raw_point, average, buzzer_status, led_status)

                #The length of the seizure is calculated by multiplying the number of cycles by the time_check
                line = "Seizure Stopped" + '\t' + str(seizure_time.strftime("%X \t %x")) + '\t' + str(seizure_length) + "s" + '\n'+'\n'
                write_to_file(line, filename_2)
                line = str(raw_point) + "\t" + str(average) + "\t" + str(status) + "\n"
                write_to_file(line, filename_1)
                break
        
            #blinking_lights functions is called to in case the number of cycles exceeds 10, the parents are alerted of a medical emergency.
            blinking_lights(cycles)


            #Re-intializes the alert_list and re-assigns the alert_list to the new values of rolling averages so that the while True loop doesn't loop with the same list.
            alert_list = []
            alert_list = alert_values()
